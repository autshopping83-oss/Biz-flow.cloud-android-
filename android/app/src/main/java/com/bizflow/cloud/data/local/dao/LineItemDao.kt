...
interface LineItemDao {
    ...
    @Query("SELECT description, SUM(quantity) as quantity, SUM(total) as total FROM line_items WHERE documentId IN (SELECT id FROM documents WHERE deletedAt IS NULL AND status IN ('PAGO') AND currency = :currency AND createdAt BETWEEN :startMs AND :endMs) GROUP BY description ORDER BY quantity DESC LIMIT :limit")
