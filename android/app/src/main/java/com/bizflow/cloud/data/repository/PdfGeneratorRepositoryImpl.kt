...
class PdfGeneratorRepositoryImpl(
    ...
    suspend fun saveAndPrintPdf(
        ...
        android.util.Log.d("PDF_PRINT", "STEP_1_PDF_GENERATION_START")
        val bytes = generatePdfBytes(html) ?: run {
            android.util.Log.e("PDF_PRINT", "STEP_1_FAILED_PDF_BYTES_NULL")
            return null
        }
        android.util.Log.d("PDF_PRINT", "STEP_2_PDF_GENERATED bytes=${bytes.size}")
        ...
        android.util.Log.d("PDF_PRINT", "STEP_3_PDF_SAVED uri=$uri")
        printPdfFromBytes(context, bytes, jobName)
        android.util.Log.d("PDF_PRINT", "STEP_4_PRINT_PDF_FROM_BYTES_RETURNED")
        return uri
    }

    private fun printPdfFromBytes(context: Context, pdfBytes: ByteArray, jobName: String) {
        android.util.Log.d("PDF_PRINT", "STEP_5_PRINT_PDF_FROM_BYTES_ENTRY")
        if (printing.get()) {
            android.util.Log.w("PDF_PRINT", "STEP_5_BLOCKED_PRINTING_ALREADY_TRUE")
            return
        }
        android.util.Log.d("PDF_PRINT", "STEP_6_POSTING_TO_MAIN_HANDLER")
        mainHandler.post {
            android.util.Log.d("PDF_PRINT", "STEP_7_MAIN_HANDLER_EXECUTING")
            if (printing.compareAndSet(false, true)) {
                android.util.Log.d("PDF_PRINT", "STEP_8_PRINTING_LOCK_ACQUIRED")
                val printManager = context.getSystemService(Context.PRINT_SERVICE) as? PrintManager
                android.util.Log.d("PDF_PRINT", "STEP_9_PRINT_MANAGER=$printManager")
                if (printManager == null) {
                    android.util.Log.e("PDF_PRINT", "STEP_9_PRINT_MANAGER_NULL")
                    printing.set(false)
                    return@post
                }
                ...
                android.util.Log.d("PDF_PRINT", "STEP_10_CALLING_PRINT_MANAGER_PRINT")
                try {
                    printManager.print(jobName, adapter, attrs)
                    android.util.Log.d("PDF_PRINT", "STEP_11_PRINT_MANAGER_PRINT_CALLED")
                } catch (e: Exception) {
                    android.util.Log.e("PDF_PRINT", "STEP_11_PRINT_FAILED", e)
                    printing.set(false)
                }
            } else {
                android.util.Log.w("PDF_PRINT", "STEP_8_PRINTING_LOCK_FAILED")
            }
        }
        android.util.Log.d("PDF_PRINT", "STEP_6B_MAIN_HANDLER_POSTED")
    }
