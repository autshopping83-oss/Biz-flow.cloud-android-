...
class PdfGeneratorRepositoryImpl(
    ...
    suspend fun saveAndPrintPdf(
        ...
        PrintDiagnostic.record("STEP_1_PDF_GENERATION_START")
        val bytes = generatePdfBytes(html) ?: run {
            PrintDiagnostic.recordError("STEP_1_FAILED_PDF_BYTES_NULL", RuntimeException("PDF bytes null"))
            return null
        }
        PrintDiagnostic.record("STEP_2_PDF_GENERATED bytes=${bytes.size}")
    ...
    private fun printPdfFromBytes(context: Context, pdfBytes: ByteArray, jobName: String) {
        PrintDiagnostic.record("STEP_5_PRINT_PDF_FROM_BYTES_ENTRY")
        PrintDiagnostic.record("PRINTING_STATE_BEFORE=${printing.get()}")

        if (printing.get()) {
            PrintDiagnostic.record("PRINT_FLOW_BLOCKED_ALREADY_PRINTING")
            return
        }
        PrintDiagnostic.record("STEP_6_POSTING_TO_MAIN_HANDLER")

        val isMain = android.os.Looper.myLooper() == android.os.Looper.getMainLooper()
        PrintDiagnostic.record("STEP_6_THREAD_IS_MAIN=$isMain")

        val postResult = mainHandler.post {
            PrintDiagnostic.record("STEP_7_MAIN_HANDLER_EXECUTING")
            PrintDiagnostic.record("PRINT_ON_MAIN_THREAD=true")

            val ctxClass = context.javaClass.name
            val isActivity = context is android.app.Activity
            val pkg = context.packageName
            PrintDiagnostic.recordContextInfo(ctxClass, isActivity, pkg)

            if (context is android.app.Activity) {
                PrintDiagnostic.record("PRINT_ACTIVITY_FINISHING=${context.isFinishing}")
                PrintDiagnostic.record("PRINT_ACTIVITY_DESTROYED=${context.isDestroyed}")
            }

            if (!printing.compareAndSet(false, true)) {
                PrintDiagnostic.record("PRINT_FLOW_BLOCKED_COMPARE_AND_SET")
                return@post
            }
            PrintDiagnostic.record("STEP_8_PRINTING_LOCK_ACQUIRED")

            PrintDiagnostic.record("PRINT_MANAGER_LOOKUP_START")
            val printManager = context.getSystemService(Context.PRINT_SERVICE) as? PrintManager
            if (printManager == null) {
                PrintDiagnostic.record("STEP_9_PRINT_MANAGER_NULL")
                printing.set(false)
                return@post
            }
            PrintDiagnostic.record("STEP_9_PRINT_MANAGER_OBTAINED class=${printManager.javaClass.name}")

            PrintDiagnostic.record("PRINT_ATTRIBUTES_CREATE_START")
            val attrs = try {
                val a = PrintAttributes.Builder()
                    .setMediaSize(PrintAttributes.MediaSize.ISO_A4)
                    .setColorMode(PrintAttributes.COLOR_MODE_COLOR)
                    .build()
                PrintDiagnostic.record("STEP_11_PRINT_ATTRIBUTES_CREATED")
                a
            } catch (t: Throwable) {
                PrintDiagnostic.recordError("PRINT_ATTRIBUTES_EXCEPTION", t)
                printing.set(false)
                return@post
            }

            PrintDiagnostic.record("PRINT_ADAPTER_CREATE_START")
            val adapter = try {
                val a = ByteArrayPrintDocumentAdapter(pdfBytes, jobName) {
                    printing.set(false)
                }
                PrintDiagnostic.record("STEP_10_PRINT_ADAPTER_CREATED")
                a
            } catch (t: Throwable) {
                PrintDiagnostic.recordError("PRINT_ADAPTER_EXCEPTION", t)
                printing.set(false)
                return@post
            }

            PrintDiagnostic.record("STEP_12_PRINT_CALL_START")
            try {
                printManager.print(jobName, adapter, attrs)
                PrintDiagnostic.record("STEP_13_PRINT_CALL_RETURNED")
            } catch (t: Throwable) {
                PrintDiagnostic.recordError("PRINT_EXCEPTION", t)
                printing.set(false)
            }
        }
        PrintDiagnostic.record("STEP_6B_MAIN_HANDLER_POSTED result=$postResult")
    }
